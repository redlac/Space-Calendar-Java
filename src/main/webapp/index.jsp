...
<html lang="en">
  ...
  <body>
	...
	<div id="month"><% Calendar cal = java.util.Calendar.getInstance();
					   int month = cal.get(Calendar.MONTH); 
					   out.println(cal.getDisplayName(month+1, 2, java.util.Locale.getDefault()) + " 2014"); %></div>
...
function previousMonth(){
    ...
    var currentMonthIndex = monthNames.indexOf(currentMonth.trim());
...
function setDayClick(calDays){
    $(".thumbnail").click(function(){
        ...
        $(this).css("border", "10px solid #9D9D9D");
...
function getAstroData(tempCalDays, filteredEvents){
    ...
    //Json data received from servlet with AJAX.
    $.get("spacecalendar").done(function(data){
    	console.log(data);
        var jsonObj = data;
        ...
        for(var i=0; i<jsonLength; i++){
            ...
            if (dayData !== undefined){
                var day = (typeof dayData[0][0] === 'string') ? dayData[0][0].trim() : dayData[0][0];
                console.log("day"+day);
                var time = dayData[0][1];
                console.log("time"+time);
                var dayEvent = (typeof dayData[0][2] === 'string') ? dayData[0][2].trim() : "";
                console.log("dayEvent"+dayEvent);
                var icon = dayData[1][0];
                ...
                if (showEvent){
                    ...
                    dayEvent = dayEvent.replace(/./g, function(char){
                        ...
                        return char.charCodeAt(0) === 65533 ? String.fromCharCode(176).charAt(0) : char; 
                    }); 
